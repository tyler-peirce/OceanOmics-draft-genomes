...
# run using: singularity run $SING/psycopg2:0.1.sif python 06_push_fastp_stats_to_sqldb.py
...
aws_stats = f"aws_stats/fastp_stats.tsv"  # if your file structure is different this might not work.
...
try:
    ...
    for index, row in aws_df.iterrows():
        ...
        INSERT INTO draft_genomes (
            og_id, seq_date, fastp_r1, fastp_r1_size, fastp_r2, fastp_r2_size
        )
        VALUES (%s, %s, %s, %s, %s, %s)
        ON CONFLICT (og_id, seq_date) DO UPDATE SET
            fastp_r1 = EXCLUDED.fastp_r1,
            fastp_r1_size = EXCLUDED.fastp_r1_size,
            fastp_r2 = EXCLUDED.fastp_r2,
            fastp_r2_size = EXCLUDED.fastp_r2_size;
        """
        params = (
            row["og_id"],  # TEXT / VARCHAR
            str(row["seq_date"]) if row["seq_date"] else None,  # TEXT or DATE
            str(row["fastp_r1"]) if row["fastp_r1"] else None,  # TEXT       
            int(row["fastp_r1_size"]) if row["fastp_r1_size"] else None,  # INTEGER
            str(row["fastp_r2"]) if row["fastp_r2"] else None,  # TEXT
            int(row["fastp_r2_size"]) if row["fastp_r2_size"] else None,  # INTEGER      
        )
